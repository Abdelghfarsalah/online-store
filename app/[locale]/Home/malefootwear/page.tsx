...
export default function MaleFootwearAllPage() {
  const locale = useLocale?.() ?? "en";
  const t = useTranslations("HomePage");
  const { data } = useGetProductsQuery("malefootwear");
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 800, margin: 0 }}>{t("allMaleFootwearTitle")}</h1>
          <p style={{ margin: "8px 0 0", color: "#6B7280" }}>{t("maleFootwearSectionTitle")}</p>
        </div>
      ...
      {!products.length ? (
        <div style={{ padding: 28, borderRadius: 18, background: "#F3F4F6", textAlign: "center", color: "#6B7280" }}>
          {t("noMaleFootwearMessage")}
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 20 }}>
          {products.map((p: Mobiles, idx: number) => {
            const title = p.Brand ?? p.brand ?? p.Description ?? p.description ?? t("productTagFallback");
